...
!
! Compile with
! gfortran -O3 -ffree-line-length-0 nmr.f90 cas.f90 -o cas
!
! Resource files
! ca.comm  conf.comm  contact.comm  profile3.comm  quasi3.comm
! r13.comm  r14.comm  r14e.comm  r14h.comm  r15.comm  r15e.comm  r15h.comm  sidechain.comm
!
! Data files
! comb.dat  comb8CA.dat combCA.dat dist.dat distL.dat exp.dat in.dd init.dat
! pair1.dat pair3.dat   par.dat    rmsinp   seq.dat
!
!        1         2         3         4         5         6         7 !
! 3456789012345678901234567890123456789012345678901234567890123456789012345678
      ...
      parameter(ndim=1000) !=1500)     !maximum length of chain-length
      ...
      common/svm1/Mcon(12,ndim),Kcon(12,ndim,100),awei(12,ndim,ndim)
      common/svm2/acc_cut,dist_svm2(12),nk_svm(3),ik_svm(3,12)
      ...
      ! ******************** NMR
      real phi(ndim),psi(ndim),phiPre(ndim),psiPre(ndim),dphiPre(ndim),dpsiPre(ndim)
      common/nmr_torsion/phi,psi,phiPre,psiPre,dphiPre,dpsiPre
      real cax_coord(ndim),cay_coord(ndim),caz_coord(ndim),nx_coord(ndim),ny_coord(ndim),nz_coord(ndim), &
           cx_coord(ndim),cy_coord(ndim),cz_coord(ndim),ox_coord(ndim),oy_coord(ndim),oz_coord(ndim)  ! cartesian coords
      common/nmr_backbone/cax_coord,cay_coord,caz_coord,nx_coord,ny_coord,nz_coord,cx_coord,cy_coord,cz_coord,ox_coord,oy_coord,oz_coord
      logical ETORSIONFLAG, ENOEFLAG, ENOEAMBIGFLAG
      real ETORSION, ETORSION_wt, ENOE, ENOEA, ENOE_wt, ENOEA_wt, TEMPLATE_scale, CORE_scale
      common/nmr_energy/ETORSION, ETORSION_wt, ENOE, ENOEA, ENOE_wt, ENOEA_wt, TEMPLATE_scale, CORE_scale, ETORSIONFLAG, ENOEFLAG, ENOEAMBIGFLAG
      real ETERMS_TRAIN(nrep,13)
      real EWTS_TRAIN(13)
      common/NMR_train/ETERMS_TRAIN,EWTS_TRAIN
      integer:: startIndex, endIndex,length
      logical:: strtokStat
      character(len=70):: line

      ! for user specified moveable regions ! limit to 100 moveable segments, where each segment is contiguous
      integer num_user_moveable
      integer user_mv_start(100)
      integer user_mv_end(100)
      common/user_moveable/num_user_moveable, user_mv_start, user_mv_end

      integer high_temp_flag   ! = 0 if normal temperature and 1 if using high temperature
      common/high_temp/high_temp_flag
      integer rand_structure  ! = 1 if use random structure in ab_initio simulation; = 0 use template
      common/rand_flag/rand_structure

      real move_acc_rate, swap_acc_rate
      common/acc_rate/move_acc_rate, swap_acc_rate

      real satis_rate
      integer violCountPrev
      integer violCountPrev2
      ! ******************** NMR
...
!cccccccccccccccccccccccc common input files cccccccccccccccccccccccccccccc
...
!cccccccccccccc sequence specified input files cccccccccccccccc///////////
...
!cccccccccccccccfor E-t cccccccccccccccccccccccccccccccccccccccccccccccccc
...
!cccccccccccccccc sequence-based contact predictions --------------->
      open(unit=50,file='conf.comm',status='unknown')
      ...
      open(unit=60,file='svmcon.dat',status='unknown')    !10
      open(unit=61,file='betacon.dat',status='unknown')   !11
      open(unit=62,file='nncom.dat',status='unknown')     !12
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

      read(19,*) hour_max

      ! ******************** NMR
      read(19,*) random,ncycle,phot,N_rep,n_run, rand_structure
      ! ******************** NMR
      ...
      ! ******************** NMR
      read(19,*) atemp2,atemp1,exc,exc1,exc2,Mend,defo_angle, high_temp_flag
      if (high_temp_flag > 0) then
      ...
      TEMPLATE_scale = 1.0
      CORE_scale = 1.0
      ! read chuan and possibly ENOE_wt, ENOEA_wt, TEMPLATE_scale, CORE_scale
      read(19,'(a)') line
      length = len(line)
      startIndex = 1
      endIndex = 1
      strtokStat = strtok(line,length, startIndex, endIndex)
      if (strtokStat) then
        read(line(startIndex:endIndex),*) chuan
        startIndex = endIndex+1
        endIndex = startIndex
        strtokStat = strtok(line,length, startIndex, endIndex)
        if (strtokStat) then
            read(line(startIndex:endIndex),*) ENOE_wt
            startIndex = endIndex+1
            endIndex = startIndex
            strtokStat = strtok(line,length, startIndex, endIndex)
            if (strtokStat) then
                read(line(startIndex:endIndex),*) ENOEA_wt
                startIndex = endIndex+1
                endIndex = startIndex
                strtokStat = strtok(line,length, startIndex, endIndex)
                if (strtokStat) then
                    read(line(startIndex:endIndex),*) TEMPLATE_scale
                    startIndex = endIndex+1
                    endIndex = startIndex
                    strtokStat = strtok(line,length, startIndex, endIndex)
                    if (strtokStat) then
                        read(line(startIndex:endIndex),*) CORE_scale
                    endif
                endif
            endif
        endif
      ...
      ! ******************** NMR
      ! user specified moveable region
      read(19,'(a)') line
      length = len(line)
      startIndex = 1
      endIndex = 1
      strtokStat = strtok(line,length, startIndex, endIndex)
      if (strtokStat) then
        read(line(startIndex:endIndex),*) num_user_moveable
        if (num_user_moveable < 1) then
            write(*,*) 'ERROR num_user_moveable', num_user_moveable, '< 1'
            stop
        endif
        do i=1,num_user_moveable
            startIndex = endIndex+1
            endIndex = startIndex
            strtokStat = strtok(line,length, startIndex, endIndex)
            read(line(startIndex:endIndex),*) user_mv_start(i)
            startIndex = endIndex+1
            endIndex = startIndex
            strtokStat = strtok(line,length, startIndex, endIndex)
            read(line(startIndex:endIndex),*) user_mv_end(i)
        enddo
      endif

      write(*,*) 'Num User Specified Moveable', num_user_moveable
      do i=1,num_user_moveable
        write(*,*) 'Moveable ', user_mv_start(i), user_mv_end(i)
      enddo
      ! ******************** NMR
...
!***
      ...
      ! ******************** NMR files ! Lch from set_common
      INQUIRE(FILE="pred.tab", EXIST=ETORSIONFLAG)
      if (ETORSIONFLAG) then
        write (*,*) 'Using pred.tab'
        call readTalos('pred.tab',phiPre,psiPre,dphiPre,dpsiPre,Lch)
      endif
      INQUIRE(FILE="noe.tbl", EXIST=ENOEFLAG)
      if (ENOEFLAG) then
         write (*,*) 'Using noe.tbl'
         call readNOEAssignments('noe.tbl')
      endif
      INQUIRE(FILE="ambig.tbl", EXIST=ENOEAMBIGFLAG)
      if (ENOEAMBIGFLAG) then
         write (*,*) 'Using ambig.tbl'
         call readAmbigNOEAssignments('ambig.tbl')
      endif

      write (*,*) 'NumNOERestraints', numRestr
      ...
      if(switch.gt.1)then
         ...
         call template_initial  !initial model from templates (M mode)
      endif
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      if(switch.gt.1)then
         write (*,*) 'template_simulation'
         call template_simulation !simulate the templates;
              ! switch=1 ab initio mode, 2=template mode
         stop
      endif
 ...
 15   i_run=i_run+1
       ...
       ! ******************** NMR
      if (rand_structure > 0) then
        call random_initial       !produce initial structure randomly.
        write (*,*) 'Using random structure for ab initio simulation'
      else
        call read_initial         !read initial (x,y,z) from 'init.dat' (A mode)
        write (*,*) 'Using templates for ab initio simulation'
      endif
      ! ******************** NMR
...
!ccc
            ...
            !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            if (itemp == 1) then
                satis_rate = 1.0-real(violCount)/real(numRestr) ! fract restr satisified

!                if (icycle.gt.1.and.satis_rate.gt.0.99999) then
!                    write (*,*) 'No more violations. Quiting early at cycle ',icycle
!                    goto 1112
!                endif

!                if (satis_rate.lt.0.2) then
!                    move_acc_rate = 4.0
!                    swap_acc_rate = 3.0
!                elseif (satis_rate.lt.0.5) then
!                    move_acc_rate = 3.0
!                    swap_acc_rate = 2.0
!                elseif (satis_rate.lt.0.8) then
!                    move_acc_rate = 2.0
!                    swap_acc_rate = 1.5
!                !elseif (violCount.gt.0.and.violCount == violCountPrev.and.violCountPrev == violCountPrev2) then
!                !    move_acc_rate = 3.0
!                !    swap_acc_rate = 2.0
!                else
!                    move_acc_rate = 1.0
!                    swap_acc_rate = 1.0
!                endif
                write(*,*) 'satis_rate, violCount, move,swap rate:',satis_rate,violCount,move_acc_rate,swap_acc_rate
                tempNOEEnergy = noeRestraintPenalty(1,Lch,.false.)

                violCountPrev2 = violCountPrev
                violCountPrev = violCount
            endif

            if (icycle == 1.and.itemp == 1) then
                write (*,*) 'Cycle:',0,NMR_ETERMS(1), violCount
            endif
 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!ccccccccccrecord energy and (x,y,z) cccccccccccccccccccc
            ...
            ! ******************** NMR
            ETERMS_TRAIN(itemp,1) = NMR_ETERMS(1)
            ETERMS_TRAIN(itemp,2) = NMR_ETERMS(2)
            ETERMS_TRAIN(itemp,3) = NMR_ETERMS(3)
            ETERMS_TRAIN(itemp,4) = NMR_ETERMS(4)
            ETERMS_TRAIN(itemp,5) = NMR_ETERMS(5)
            ETERMS_TRAIN(itemp,6) = NMR_ETERMS(6)
            ETERMS_TRAIN(itemp,7) = NMR_ETERMS(7)
            ETERMS_TRAIN(itemp,8) = NMR_ETERMS(8)
            ETERMS_TRAIN(itemp,9) = NMR_ETERMS(9)
            ETERMS_TRAIN(itemp,10) = NMR_ETERMS(10)
            ETERMS_TRAIN(itemp,11) = NMR_ETERMS(11)
            ETERMS_TRAIN(itemp,12) = NMR_ETERMS(12)
            ETERMS_TRAIN(itemp,13) = NMR_ETERMS(13)
            ! ******************** NMR
...
!ccccccccccccccccccccc snapshots of E(1), E(N_rep) ccccccccccccc
         if(icycle.eq.icycle/1*1)then
            ...
            do k=1,n_repf
               ...
               ! ******************** NMR
               write(30+k,4011)Lch,E_rep(k),i_tr,icycle,&
                  ETERMS_TRAIN(k,1),ETERMS_TRAIN(k,2),ETERMS_TRAIN(k,3),ETERMS_TRAIN(k,4),&
                  ETERMS_TRAIN(k,5),ETERMS_TRAIN(k,6),ETERMS_TRAIN(k,7),ETERMS_TRAIN(k,8),&
                  ETERMS_TRAIN(k,9),ETERMS_TRAIN(k,10),ETERMS_TRAIN(k,11),ETERMS_TRAIN(k,12),&
                  ETERMS_TRAIN(k,13), &
                  EWTS_TRAIN(1),EWTS_TRAIN(2),EWTS_TRAIN(3),EWTS_TRAIN(4),EWTS_TRAIN(5),&
                  EWTS_TRAIN(6),EWTS_TRAIN(7),EWTS_TRAIN(8),EWTS_TRAIN(9),EWTS_TRAIN(10),&
                  EWTS_TRAIN(11),EWTS_TRAIN(12), EWTS_TRAIN(13)
              ! ******************** NMR
 ...
 4011    format(i8,1x,f10.1,2i8,1x,13f11.3,13f11.4)
 402     format(f10.3,1x,f10.3,1x,f10.3)
         ...
         call count_restrains   !count number of satisfied restraints
          ...
          !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!          write (*,*) 'Cycle:',icycle,noeEnergy1, numViol1
 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!ccccccccccccccccc swap replicas cccccccccccccccccccccccccccccccc
 ...
 1112 if(i_run.lt.n_run)goto 15
 901  continue
...
!ccccccccccccccccccccccc <E>, NNa/NNt ccccccccccccccccccccccccccc
      ...
      write(20,*)'i_rep  T   <E>   NNa(i_temp)  NNt(i_temp)  Na/Nt     <ESHORT12>'
      do i=1,N_rep
...
!ccccccccccreset restraints weights according to zscore cccccccccccc
      ...
      INQUIRE(FILE="noe.tbl", EXIST=ENOEFLAG)
      if (ENOEFLAG) then
        er1 = er1*TEMPLATE_scale ! dist.dat
        er3 = er3*TEMPLATE_scale ! comb.dat
        er4 = er4*TEMPLATE_scale ! comb.dat of dev
        er5 = er5*TEMPLATE_scale ! combCA.dat
        er6 = er6*TEMPLATE_scale ! distL
        eh1c = eh1c*TEMPLATE_scale ! par.dat
        er7 = 5.0 ! not used ! easy case 500, med case 100, hard case 5, ! deviation from template
        ! 1/r terms
        eh1 = eh1*CORE_scale ! ca-sc
        eh1a = eh1a*CORE_scale ! non-parallel ca-ca
        eh1b = eh1b*CORE_scale ! sc-sc
        eh4 = eh4*CORE_scale ! parallel ca-ca

        ETORSION_wt = 0 ! not used
        ! set when read line with chuan ! ENOE_wt = 5 ! 10 ! 5
        ! set when read line with chuan ! ENOEA_wt = 50.0 ! 2000.0 ! 50.0
      else
        ETORSION_wt = 0
        ENOE_wt = 0
        ENOEA_wt = 0
      ...
      write(*,*) 'ENOE_wt=',ENOE_wt, 'ENOEA_wt=',ENOEA_wt, 'TEMPLATE_scale=',TEMPLATE_scale, 'CORE_scale',CORE_scale

      ! record the weights
      EWTS_TRAIN(1) = ENOE_wt
      EWTS_TRAIN(2) = er1
      EWTS_TRAIN(3) = er3
      EWTS_TRAIN(4) = er4
      EWTS_TRAIN(5) = er5
...
!c     (from NMR or threading predictions or clusters)
...
!c     CAcontact restrain is only on CA.
      ...
      common/svm2/acc_cut,dist_svm2(12),nk_svm(3),ik_svm(3,12)
      common/svm4/acc_cutB,acc_cutG
      DIMENSION r1(8000),r2(8000),conf0(12)
...
!ccc   read conf.comm ------->
 10   read(50,*)acc0
      ...
      do k=1,12
         if(k.lt.12)then
            read(50,*)conf0(k),dist,i
         else                   !nncom.dat
            conf0(k)=conf0(11)  !same as betacon
            dist=8
            i=1
         endif
         dist_svm2(k)=(dist/0.87)**2
         if(i.eq.1)then         !CA
            nk_svm(1)=nk_svm(1)+1
            ik_svm(1,nk_svm(1))=k
         endif
         if(i.eq.2)then         !CB
            nk_svm(2)=nk_svm(2)+1
            ik_svm(2,nk_svm(2))=k
            conf0(k)=conf0(k)*acc_cutB
         endif
         if(i.eq.3)then         !SG
            nk_svm(3)=nk_svm(3)+1
            ik_svm(3,nk_svm(3))=k
            conf0(k)=conf0(k)*acc_cutG
         endif
      enddo
      
!ccc   read contact predictions ------->
      n_cut=Lch*0.2             ! using at least 20%
      n_cut1=Lch*0.5            ! using at least 50%
      do k=1,12
         i_c=0
         read(50+k,*,end=14)ntmp
         if(ntmp.gt.8000)ntmp=8000
         do i=1,ntmp
            ...
            if(k.eq.10)then     !svmcon
               conf=2.0*(conf-0.5) !convert [0.5, 1]->[0, 1]
            endif
            if(conf.gt.1)conf=1
            mk=0
            if(i_c.lt.n_cut)then !n_cut=L*0.2
               mk=1
            else
               if(k.eq.3.or.k.eq.6.or.k.eq.9.or. &
                    k.eq.10.or.k.eq.11.or.k.eq.12)then !dist=8
                  if(i_c.lt.n_cut1)then !n_cut1=L*0.5
                     mk=1
                  endif
               else             !dist=6 and 7
                  if(conf.gt.conf0(k))then
                     mk=1
                  endif
               endif
            endif
            if(mk.eq.1)then
               ...
               r1(i_c)=i1
               r2(i_c)=i2
               awei(k,i1,i2)=1+(conf-conf0(k))**1
               awei(k,i2,i1)=awei(k,i1,i2)
            endif
         enddo
 14      NcomCA=i_c
         write(*,*)k,i_c,n_cut,conf0(k)
...
!ccc   output restraints------------->
!c         write(*,*)'cutoff=',k,conf0(k)
!c         write(*,*)'Number of restraints:',NcomCA
!c         nnc=0
!c         do i=1,Lch
!c            nnc=nnc+Mcon(k,i)
!c            write(*,12)i,Mcon(k,i),(Kcon(k,i,j),j=1,Mcon(k,i))
!c 12         format(i4,'(',i2,'):',20i4)
!c            write(*,13)i,Mcon(k,i),(awei(k,i,Kcon(k,i,j)),j=1,Mcon(k,i))
!c 13         format(i4,'(',i2,'):',20f8.5)
!c         enddo
!c         write(*,*)'Number of CAcontact=',nnc,' Lch=',Lch
!c         write(*,*)
...
!cccccccccccccccccccset temperature for different replicas ccccc
      ...
      ! ******************** NMR
      real hi ! scaled i to higher range [N_REP/2, N_REP] instead of 1 to N_REP
      integer:: high_temp_flag   ! = 0 if normal temperature and 1 if using high temperature
      common/high_temp/high_temp_flag
      ! ******************** NMR
...
!***********for normal run ***********************************
      if (N_REP > 1) then
        do i=1,N_REP
            if (high_temp_flag > 0) then
                hi = ( (N_REP-N_REP/2.0)*(i-1.0)/(N_REP-1.0) ) + N_REP/2.0
                aT_rep(i)=atemp1*(atemp2/atemp1)**((hi-1)/(N_rep-1))
                aTs_rep(i)=aTs1*(aTs2/aTs1)**((hi-1)/(N_rep-1))
                aTTs_rep(i)=aTTs1*(aTTs2/aTTs1)**((hi-1)/(N_rep-1))
            else
...
!**********************************************************
      if(ssp.eq.1)then
         write (*,*) 'Using predicted secondary structures'
...
!cTTTccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      ! ******************** NMR
      integer:: num_user_moveable
      integer:: user_mv_start(100), user_mv_end(100)
      common/user_moveable/num_user_moveable, user_mv_start, user_mv_end
      ! ******************** NMR
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      ! ******************** NMR
      ! ******************** skip this part if moveable regions were specified
      ! ******************** skip cut 5 loops for full template
      ! ******************** skip secondary and skip fill gaps
      if (num_user_moveable.gt.0) then
        write(*,*) 'User specified moveable regions. Skipping cut 5 loops'
        goto 876
      endif
      ! ******************** END NMR


      mloop=5 ! mloop is not used outside of template_initial
...
!ccc   check loop position & loop number
         nloop=0 ! total number of loop segments
         if(sec(1).eq.1)then
            nloop=nloop+1  ! n-term loop
            loop_i(nloop)=1 ! beginning of loop
            loop_f(nloop)=1 ! end of loop
         endif
         do i=2,Lch
         ...
         enddo
         do i=1,nloop
            loop_p(i)=int(float(loop_f(i)+loop_i(i))/2.0) ! loop midpt
         enddo
!ccc   cut mloop by setting q(i)=0
         if(mloop.gt.0)then ! mloop is 5 from above
            delt=float(nloop)/float(mloop+1)
            if(delt.lt.1)delt=1
            kk=1
            do i=1,nloop  ! if > 5 loops, get 5 loops by evenly taking every other loop
      ...
      endif ! end if full length model
 50   continue
...
!***   1: ras => gap +- 2, smallest moveable-length is 3
    ...
    ! ******************** NMR
    ! ******************** cut these regions instead
 877  if (num_user_moveable.gt.0) then
        nfl=0 ! num moveable residues
        do i=1,Lch
            ras(i)=0
        enddo
        do i=1,num_user_moveable
            ...
            i2 = user_mv_end(i)
            do ii=i1,i2
                ! write(*,*) 'Setting user specified moveable region: ', i1, i2
                nfl=nfl+1
                ras(nfl)=ii
            enddo
        enddo
    ...
    ! ******************** END NMR
      ...
      if(i_bigbond.eq.1)then    !only for medium (i_bigbond set in set_common)
...
!cTTTTTTTTTTccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      do 1111 icycle=1,ncycle
         do 2222 itemp=1,N_rep  !iterate for all the replicas
          ...
          if (icycle == 1.and.itemp == 1) then
            write (*,*) 'Cycle:',0,NMR_ETERMS(1), violCount
            ! call write_pdb(.false.,'out.pdb')
          endif
 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

!ccc
            do 3333 iphot=1,phot !N_swap, iterate at fixed temperature
               do 4444 i_nfl=1,nfl ! nfl=Lch
                  fff=aranzy(nozy)
                  if(fff.le.bh2)then
                     call move2
                  elseif(fff.le.bh3s)then
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
                  atime=second()/3600.0
                  if(atime.gt.hour_max)goto 901
 4444           continue ! END for each nfl moves at fixed temp (chain len dependent)
 3333         continue  ! END for each phot moves at fixed temperature
!ccc
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            enddo
!c^^^^^^^^^^^^record done ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
 2222    continue ! end for each temperature itemp
...
!      if (ETORSIONFLAG) then
          ! build backbone
          ! get the CAs +/- 3 each side of iiii,jjjj
          s = iiii-3
          e = jjjj+3
          if (s < 1) then
              s = 1
          ...
          do i=s,e
              iseq=seq(i)
              if(mv(i).gt.0)then
                  cax_coord(i)=x(i)*0.87 ! use cartesian coords instead of grid
                  cay_coord(i)=y(i)*0.87
                  caz_coord(i)=z(i)*0.87
              else
                  cax_coord(i)=ex(i)*0.87
                  cay_coord(i)=ey(i)*0.87
                  caz_coord(i)=ez(i)*0.87
              endif
          end do
! RJ          call build_bb_ij_nocheck(cax_coord,cay_coord,caz_coord,nx_coord,ny_coord,nz_coord, &
! RJ         cx_coord,cy_coord,cz_coord,ox_coord,oy_coord,oz_coord,s,e,Lch,seq) ! requires pulchra.dat

!          call getPhiPsi_ij(cax_coord,cay_coord,caz_coord,nx_coord,ny_coord,nz_coord, &
!          cx_coord,cy_coord,cz_coord,phi,psi,s,e,Lch)
!          ETORSION = torsionRestraintPenalty(phi,psi,phiPre,psiPre,s,e,Lch)
      endif
      if (ENOEFLAG) then
          ENOE = noeRestraintPenalty(iiii,jjjj,.false.)
          ENOEA = 0 ! noeRestraintPenaltyAnti(iiii,jjjj)
      endif
    ! ******************** NMR
...
!c^^^^^^^^^^^^^^^^^^ CAcontact restrains finished ^^^^^^^^^^^^^^^^^^^^^
             ...
             ! *****************************
             !write(*,*) 'NOEEnergy',ENOE
             !write(*,*) 'EShort ',ESHORT4
             !write(*,*) 'ETorsion ',ETORSION
             !stop
             ! ******************************
...
!c     the penalty was not counted in Enew, the following is the difference
!c     in penalty between new and old conformation, i.e. dE=Enew-Eold. Since
!c     this part energy appear only at Eold, the sign is opposite:
!     i.e. in Metropolis test, we want Enew-Eold, but
!     Enew' was computed instead for new conformation (without the penalty)
!     Therefore, we compute Eold' instead which is equal to Eold'-penalty for new conformation
!     Thus Enew'-Eold' = Enew'+penalty-Eold = Enew-Eold
...
!*^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
      ...
      ESHORT= &
           ...
           +er23*ESHORT23 &
      ! ******************** NMR
           +ETORSION_wt*ETORSION &
           +ENOE_wt*ENOE &
           +ENOEA_wt*ENOEA

      NMR_ETERMS(1) = ENOE
      NMR_ETERMS(2) = ESHORT3
      NMR_ETERMS(3) = ESHORT4
      NMR_ETERMS(4) = ESHORT4a
      NMR_ETERMS(5) = ESHORT9
      NMR_ETERMS(6) = ESHORT10
      NMR_ETERMS(7) = ESHORT11
      NMR_ETERMS(13) = ENOEA
      ! ******************** NMR
...
!c^^^^^^^^ four-bonds movement conformation finished ^^^^^^^^^^
      if(nn(m).eq.ica(m).and.nn(m1).eq.ica(m1).and.nn(m2).eq.ica(m2))goto 113
...
!c     w_12=wi(j)wj(i)/wi(i)wj(j) ------------->
      ...
      if(aranzy(nozy).le.swap_acc_rate*aweight)then !!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!cccccccccccccccc Metropolis: id=1, accepted; id=3, rejected ccccccccccccccccc
      ...
      id=1
      if(dE.gt.0)then
         if(aranzy(nozy).gt.move_acc_rate*weight12(dE))then !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            id=3
         endif
      endif
      return
...
!c   can be replaced by any other suitable generator for random numbers)
 ...
 2    ix1=mod(ia1*ix1+ic1,m1)
      ...
      ! writes coordinates of current replica to file. If ca flag is .true., only ca atoms are printed
      subroutine write_pdb(caflag,outf)
          use nmr
          implicit none
          integer, parameter:: ndim=1000
          integer, parameter:: nvec=416
          logical,intent(in)::caflag
          ...
          common/aminoacid/sequ
          integer mv(ndim)
          common/chainm/mv
          integer ica(0:ndim)
          integer x(ndim), y(ndim), z(ndim)
          common/chain1/ica,x,y,z
          real ex(ndim), ey(ndim), ez(ndim)
          common/echain1/ex,ey,ez !Ca
          real egx(ndim),egy(ndim),egz(ndim)
          common/echain2/egx,egy,egz !SG
          real gx(nvec,nvec,0:19),gy(nvec,nvec,0:19),gz(nvec,nvec,0:19)
          common/sg/gx,gy,gz
          real etx(ndim),ety(ndim),etz(ndim)
          common/echain6/etx,ety,etz !CB
          real hx(nvec,nvec,0:19),hy(nvec,nvec,0:19),hz(nvec,nvec,0:19)
          common/cb/hx,hy,hz

          real cax_coord(ndim),cay_coord(ndim),caz_coord(ndim),nx_coord(ndim),ny_coord(ndim),nz_coord(ndim), &
              cx_coord(ndim),cy_coord(ndim),cz_coord(ndim),ox_coord(ndim),oy_coord(ndim),oz_coord(ndim)
          common/nmr_backbone/cax_coord,cay_coord,caz_coord,nx_coord,ny_coord,nz_coord, &
              cx_coord,cy_coord,cz_coord,ox_coord,oy_coord,oz_coord

          real xca,yca,zca,xcb,ycb,zcb,xsc,ysc,zsc,xn,yn,zn
          real axi,ayi,azi ! used for SC
          integer iseq,nv1,nv2! used for CB, SC
          integer serial,i
          ...
          do i=1,Lch
              ...
              if (caflag) then
                  write(987,988) 'ATOM  ',serial,'CA ',sequ(i),'A',i,xca,yca,zca
                  serial = serial+1
              else
                  if(mv(i).gt.0)then
                      xcb=x(i)+HX(ica(i-1),ica(i),iseq)
                      ycb=y(i)+HY(ica(i-1),ica(i),iseq)
                      zcb=z(i)+HZ(ica(i-1),ica(i),iseq)
                  else
                      xcb=etx(i)
                      ycb=ety(i)
                      zcb=etz(i)
                  ...
                  if(mv(i).gt.0)then
                      ...
                      nv1=ica(i-1)
                      nv2=ica(i)
                      xsc=axi+GX(nv1,nv2,iseq)
                      ysc=ayi+GY(nv1,nv2,iseq)
                      zsc=azi+GZ(nv1,nv2,iseq)
                  else
                      xsc=egx(i)
                      ysc=egy(i)
                      zsc=egz(i)
                  ...
                  write(987,988) 'ATOM  ',serial,'N  ',sequ(i),'A',i,xn,yn,zn
                  serial = serial+1
                  write(987,988) 'ATOM  ',serial,'CA ',sequ(i),'A',i,xca,yca,zca
                  serial = serial+1
                  if (sequ(i) .ne. 'GLY') then
                      write(987,988) 'ATOM  ',serial,'CB ',sequ(i),'A',i,xcb,ycb,zcb
                      serial = serial+1
                      if (sequ(i) .ne. 'ALA') then
                          write(987,988) 'ATOM  ',serial,'CG ',sequ(i),'A',i,xsc,ysc,zsc
                          serial = serial+1
                      endif
                  endif
              endif
          end do
          write(987,989) 'TER   ',serial,sequ(Lch),'A',Lch
          write(987,*) 'END'
          close(987)

          ! PDB format
          ! We consider only up to col 54
          ! 1 – 6     Record name e.g. ATOM
...
989       format(a6,i5,6x,a3,1x,a1,i4) ! TER
